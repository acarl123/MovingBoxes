...
class FindNodeDlg(wx.Dialog):
	def __init__(self, parent, efd):
		...
		#self.ColoumnSorter = listmix.ColumnSorterMixin(4)
		...
		#self.Bind (wx.EVT_LIST_ITEM_RIGHT_CLICK, self.OnItemRightClick,self.Results)
	...
	def DoSearch (self,e):
		...
		for bo in self.efd.getAllBusinessObjects ():
			if (fnmatch.fnmatch(efbo.getName (bo),name)):
				self.SearchBOs.add (efbo.getFirstRevision (bo,self.efd.getAllBusinessObjects()))
	...
	#def OnItemRightClick (self, e) e.GetItem()